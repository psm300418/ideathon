...
안드로이드 백그라운드 녹음, 전체 스크립트 저장, 꼰BTI 유형 분석 MVP입니다.
...
- 기능 4: 저장된 스크립트를 로컬 규칙 기반으로 분석해 꼰BTI 유형과 꼰대력 점수를 제공합니다.
...
4. 앱의 `설정` 탭에서 `접근성` 버튼을 눌러 `꼰BTI 볼륨 버튼 감지` 서비스를 켭니다.
...
- `MainActivity`: 녹음/스크립트, 요약, 꼰BTI 측정, 설정 탭 UI
...
- `analysis/NaggingAnalyzer`: 무료 로컬 규칙 기반 꼰BTI 분석기
- `analysis/NaggingAnalysis`: 꼰BTI 분석 결과 모델
...
꼰BTI 분석 결과는 `RecordingSession.naggingAnalysis`에 JSON 문자열로 저장됩니다. `다시 측정`을 누르면 기존 결과를 덮어씁니다.
